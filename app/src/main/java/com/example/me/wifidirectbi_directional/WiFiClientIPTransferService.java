...
public class WiFiClientIPTransferService extends IntentService{
    ...
    private static String TAG = "WiFiClientIPTransfer";
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        if (intent.getAction().equals(FileTransferService.ACTION_SEND_FILE)) {
            ...
            Log.d(WiFiClientIPTransferService.TAG,"host address: " + host);
            ...
            try {
                ...
                Log.d(WiFiClientIPTransferService.TAG, "Opening client socket for First time");
                socket.bind(null);
                socket.connect((new InetSocketAddress(host, port)), FileTransferService.SOCKET_TIMEOUT);
                Log.d(WiFiClientIPTransferService.TAG, "Client socket - " + socket.isConnected());
                ...
                */
                ...
                Log.e(WiFiClientIPTransferService.TAG, e.getMessage());
                e.printStackTrace();
            } finally {
                if (socket != null) {
                    if (socket.isConnected()) {
                        try {
                            Log.d(WiFiClientIPTransferService.TAG,"First connection Server socket closed");
